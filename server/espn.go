...
type Poller struct {
	...
	pusher   *Pusher
	mu       sync.Mutex
	lastTick map[string]int64 // poolID -> last bubble snapshot ts
}
...
func (p *Poller) pollPool(ctx context.Context, id string) error {
	...
	// Append to bubble history at most every BubbleIntervalSec.
	...
	if (now-last) >= intervalSec && live.State == "in" {
	...
	// Auto-record final winner when the game flips to post for the first time.
